(function () {
  ...
  const CONFIG = {
    storageKey: "uvlt_testlet_cat_github_pages_v1",
  ...
  function handlePopState() {
    ...
    window.history.pushState({ uvltGuard: true, sessionId: state.session ? state.session.sessionId : null }, "", window.location.href);
    showSafetyStatus("受験中はブラウザの「戻る」や再読み込みをしないでください。現在の解答状態は保持されています。", true);
  }
  ...
  function handleVisibilityChange() {
    ...
    if (state.pageHiddenDuringSession && !state.session.completedAt) {
      ...
      showSafetyStatus("テスト画面に戻りました。途中状態は保存されています。続けて解答してください。", false);
    }
  }
  ...
  function handlePageShow(event) {
    ...
    if (event.persisted) {
      ...
      showSafetyStatus("画面が復元されました。保存済みの状態から続行します。", false);
    }
  }
  ...
  function approximatePatternSecondDerivatives(theta, testlet) {
    const baseStep = 0.1;
    const step = Math.min(baseStep, theta - CONFIG.thetaMin, CONFIG.thetaMax - theta);

    if (step <= 1e-6) {
      return new Array(testlet.patternCount).fill(0);
    }

    const lowerTheta = theta - step;
    const upperTheta = theta + step;
    const centerLikelihoods = getPatternLikelihoodsAtTheta(testlet, theta);
    const lowerLikelihoods = getPatternLikelihoodsAtTheta(testlet, lowerTheta);
    const upperLikelihoods = getPatternLikelihoodsAtTheta(testlet, upperTheta);
    const denominator = step * step;
    const secondDerivatives = [];

    for (let patternIndex = 0; patternIndex < testlet.patternCount; patternIndex += 1) {
      secondDerivatives.push(
        (Math.log(upperLikelihoods[patternIndex]) - (2 * Math.log(centerLikelihoods[patternIndex])) + Math.log(lowerLikelihoods[patternIndex])) / denominator
      );
    }
    ...
    const ranked = candidatePool
      .map(function (testlet) {
        ...
        return {
          ...
          compositeScore: primaryMetricScore - coveragePenalty - outOfBandPenalty,
  ...
  function simulateSingleSession(profile, rng) {
    const thetaTrue = clamp(sampleNormal(rng, profile.thetaMean, profile.thetaSD), CONFIG.thetaMin, CONFIG.thetaMax);
    const responses = [];
  ...
  function summarizeSimulationProfile(profile, rows) {
    return {
      ...
      avgTestlets: Number(mean(rows.map(function (row) { return row.administeredTestlets; })).toFixed(2)),
      minTestlets: Math.min.apply(null, rows.map(function (row) { return row.administeredTestlets; })),
      maxTestlets: Math.max.apply(null, rows.map(function (row) { return row.administeredTestlets; })),
      ...
      avgThetaBias: Number(mean(rows.map(function (row) { return row.thetaEstimated - row.thetaTrue; })).toFixed(3)),
      avgAbsThetaError: Number(mean(rows.map(function (row) { return Math.abs(row.thetaEstimated - row.thetaTrue); })).toFixed(3)),
  ...
  function withTemporaryConfig(overrides, task) {
    ...
    try {
      ...
      CONFIG.minTestlets = originalConfig.minTestlets;
      CONFIG.maxTestlets = originalConfig.maxTestlets;
      CONFIG.targetSE = originalConfig.targetSE;
    }
  }
  ...
  function finishSession(stopReason) {
    ...
    logSessionEvent("session_completed", stopReason);
    renderResults();
    persistState();
    setScreen("result");
    autoDownloadExcel();
  ...
  function renderExportStatus() {
    ...
    const rows = [
      ...
      {
        title: "この端末の一時保存",
        value: state.storageCleared ? "削除済み" : "保存中",
        note: state.storageCleared
  ...
  function autoDownloadExcel() {
    ...
    try {
      ...
      persistState();
      renderExportStatus();
      setStatus(filename + " を自動保存しました。");
    } catch (error) {
      setStatus("Excel の自動保存に失敗しました。結果画面のボタンから再保存してください。");
    }
  }
